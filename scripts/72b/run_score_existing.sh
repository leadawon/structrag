...
# Re-run scoring (structured eval + LLM judge + Loong metrics) on a completed
# inference run without re-running inference.  The judge server must be
# running (START_SERVER=1 starts it automatically).
...
DEFAULT_API_MODEL_NAME="${DEFAULT_API_MODEL_NAME:-Qwen2-72B-Instruct}"
...
usage() {
    cat <<EOF
Usage:
  bash scripts/72b/run_score_existing.sh --latest
  bash scripts/72b/run_score_existing.sh --output-path-suffix '_ts-...'
  bash scripts/72b/run_score_existing.sh --manifest /abs/path/run_manifest.json
  START_SERVER=1 bash scripts/72b/run_score_existing.sh --latest
  DRY_RUN=1 bash scripts/72b/run_score_existing.sh --latest
...
Behavior:
  ...
  - If START_SERVER=1, starts the 72B local judge server first and stops it afterwards

Defaults:
  DEFAULT_DATASET_NAME=loong
  DEFAULT_LLM_NAME=qwen
  DEFAULT_API_MODEL_NAME=Qwen2-72B-Instruct
  FORCE_OVERWRITE=1
  START_SERVER=0
...
mapping = {
    ...
    "MANIFEST_API_MODEL_NAME": data.get("api_model_name", "Qwen2-72B-Instruct"),
