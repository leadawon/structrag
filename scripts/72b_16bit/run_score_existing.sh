...
# Re-run scoring on a completed fp16 inference run without re-running inference.
...
DEFAULT_API_MODEL_NAME="${DEFAULT_API_MODEL_NAME:-Qwen2-72B-Instruct-fp16}"
...
usage() {
    cat <<EOF
Usage:
  bash scripts/72b_16bit/run_score_existing.sh --latest
  bash scripts/72b_16bit/run_score_existing.sh --output-path-suffix '_ts-...'
  bash scripts/72b_16bit/run_score_existing.sh --manifest /abs/path/run_manifest.json
  START_SERVER=1 bash scripts/72b_16bit/run_score_existing.sh --latest
  DRY_RUN=1 bash scripts/72b_16bit/run_score_existing.sh --latest

Defaults:
  DEFAULT_DATASET_NAME=loong
  DEFAULT_LLM_NAME=qwen
  DEFAULT_API_MODEL_NAME=Qwen2-72B-Instruct-fp16
  FORCE_OVERWRITE=1
  START_SERVER=0
  SERVER_SCRIPT_PATH=$ROOT_DIR/scripts/72b_16bit/run_server.sh
EOF
}
...
mapping = {
    ...
    "MANIFEST_API_MODEL_NAME": data.get("api_model_name", "Qwen2-72B-Instruct-fp16"),
