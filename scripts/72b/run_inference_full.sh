...
# Run StructRAG on the full Loong dataset (all 1600 samples) with Qwen2-72B-Instruct.
#
...
OUTPUT_PATH_SUFFIX="${OUTPUT_PATH_SUFFIX:-qwen2-72b-full}"
...
# Full dataset: 1600 samples split across 8 workers × 200 items each
...
Behavior:
  ...
  - Writes experiment logs and results under eval_results/qwen/loong<suffix>/
...
Examples:
  bash scripts/72b/run_inference_full.sh
  bash scripts/72b/run_inference_full.sh --logging
  FORCE_NEW_RUN=1 bash scripts/72b/run_inference_full.sh
  OUTPUT_PATH_SUFFIX=qwen2-72b-full-v2 bash scripts/72b/run_inference_full.sh
EOF
}
...
if ! model_ready "$MODEL_DIR"; then
    ...
    echo "  MODEL_DIR=/path/to/Qwen2-72B-Instruct bash scripts/72b/run_inference_full.sh"
    exit 1
fi
...
echo "Starting full Loong dataset run with Qwen2-72B-Instruct."
...
# all_workers runs worker_id 0..WORKER_COUNT-1 sequentially with --no_shuffle,
# covering all 1600 samples (8 workers × 200 items each) exactly once.
