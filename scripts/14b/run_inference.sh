...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    cat <<EOF
Usage:
  bash scripts/14b/run_inference.sh
  bash scripts/14b/run_inference.sh sample5
  bash scripts/14b/run_inference.sh sample100
  bash scripts/14b/run_inference.sh sample9999
  bash scripts/14b/run_inference.sh single <dataset_id>

Behavior:
  - No args: runs sample5 then sample100
  - With args: forwards to run_inference.sh using the 14B model defaults
...
if [[ ! -e "$MODEL_DIR" ]]; then
    ...
    echo "Example:"
    echo "  MODEL_DIR=$ROOT_DIR/model/Qwen2.5-14B-Instruct bash scripts/14b/run_inference.sh sample5"
    echo "  MODEL_DIR=$ROOT_DIR/model/Qwen2.5-14B-Instruct bash scripts/14b/run_inference.sh"
    exit 1
fi
