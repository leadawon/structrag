...
DEFAULT_LOG_PATH="$ROOT_DIR/logs/qwen35_27b_vllm_gpu4567.log"
DEFAULT_PID_FILE="$ROOT_DIR/logs/qwen35_27b_vllm_gpu4567.pid"
...
usage() {
    cat <<EOF
Usage:
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh --detach
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh --stop
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh --logging
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh --logging --detach
  bash scripts_full/qwen3p5_27b_vllm/run_server.sh --logging --stop
...
Logging mode:
  STRUCTRAG_LOGGING_ROOT=$ROOT_DIR/27b_vllm_full_logging
EOF
}
...
if [[ ! -e "$MODEL_DIR" ]]; then
    ...
    echo "Example:"
    echo "  MODEL_DIR=/path/to/Qwen3.5-27B bash scripts_full/qwen3p5_27b_vllm/run_server.sh"
    echo "  MODEL_DIR=/path/to/Qwen3.5-27B bash scripts_full/qwen3p5_27b_vllm/run_server.sh --detach"
    exit 1
fi
