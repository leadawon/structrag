...
# Qwen2-72B-Instruct served in float16 precision (8 GPUs).
# Model files are identical to scripts/72b/; only DTYPE differs (float16 vs bfloat16).
...
DEFAULT_LOG_PATH="$ROOT_DIR/logs/qwen2_72b_fp16_vllm.log"
DEFAULT_PID_FILE="$ROOT_DIR/logs/qwen2_72b_fp16_vllm.pid"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-32768}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-Qwen2-72B-Instruct-fp16}"
DTYPE="${DTYPE:-float16}"
...
usage() {
    cat <<EOF
Usage:
  bash scripts/72b_16bit/run_server.sh
  bash scripts/72b_16bit/run_server.sh --detach
  bash scripts/72b_16bit/run_server.sh --stop
  bash scripts/72b_16bit/run_server.sh --logging
  bash scripts/72b_16bit/run_server.sh --logging --detach
  bash scripts/72b_16bit/run_server.sh --logging --stop
...
Defaults:
  ...
  MAX_MODEL_LEN=32768
  SERVED_MODEL_NAME=Qwen2-72B-Instruct-fp16
  DTYPE=float16
...
Logging mode:
  STRUCTRAG_LOGGING_ROOT=$ROOT_DIR/72b_16bit_logging
EOF
}
...
for arg in "$@"; do
    case "$arg" in
        --help|-h) usage; exit 0 ;;
        --detach) DETACH_MODE=1; FORWARDED_ARGS+=("$arg") ;;
        --stop)   STOP_MODE=1;   FORWARDED_ARGS+=("$arg") ;;
...
if [[ "$DETACH_MODE" -eq 1 && "$STOP_MODE" -eq 1 ]]; then
    echo "--detach and --stop cannot be used together."; exit 1
fi
...
if [[ ! -e "$MODEL_DIR" ]]; then
    ...
    echo "Download first (or reuse from scripts/72b/):"
    echo "  bash scripts/72b_16bit/download_model.sh"
    exit 1
fi

if [[ ! -x "$SERVER_PYTHON_BIN" ]]; then
    echo "Server Python binary not found: $SERVER_PYTHON_BIN"; exit 1
fi
