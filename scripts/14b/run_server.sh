...
LOG_PATH="${LOG_PATH:-$ROOT_DIR/logs/qwen14b_vllm.log}"
PID_FILE="${PID_FILE:-$ROOT_DIR/logs/qwen14b_vllm.pid}"

if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    cat <<EOF
Usage:
  bash scripts/14b/run_server.sh
  bash scripts/14b/run_server.sh --detach
  bash scripts/14b/run_server.sh --stop
...
if [[ ! -e "$MODEL_DIR" ]]; then
    ...
    echo "Example:"
    echo "  MODEL_DIR=$ROOT_DIR/model/Qwen2.5-14B-Instruct bash scripts/14b/run_server.sh"
    echo "  MODEL_DIR=$ROOT_DIR/model/Qwen2.5-14B-Instruct bash scripts/14b/run_server.sh --detach"
    exit 1
fi
