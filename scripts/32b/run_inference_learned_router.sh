...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    cat <<EOF
Usage:
  bash scripts/32b/run_inference_learned_router.sh
  bash scripts/32b/run_inference_learned_router.sh sample5
  bash scripts/32b/run_inference_learned_router.sh sample100
  bash scripts/32b/run_inference_learned_router.sh sample9999
  bash scripts/32b/run_inference_learned_router.sh single <dataset_id>

Behavior:
  - No args: runs sample5 then sample100
  - Uses the main 32B model plus an external learned router at \$ROUTER_URL
