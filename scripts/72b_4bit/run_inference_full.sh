...
# 8 workers together cover every sample exactly once with --no_shuffle.
#
# The same vLLM server is reused as the LLM judge in the scoring step.
...
API_MODEL_NAME="${API_MODEL_NAME:-Qwen2-72B-Instruct-AWQ}"
STRUCTRAG_ENABLE_THINKING="${STRUCTRAG_ENABLE_THINKING:-0}"
OUTPUT_PATH_SUFFIX="${OUTPUT_PATH_SUFFIX:-qwen2-72b-4bit-full}"
QUANTIZATION="${QUANTIZATION:-awq}"
SERVER_SCRIPT_PATH="${SERVER_SCRIPT_PATH:-$ROOT_DIR/scripts/72b_4bit/run_server.sh}"
SERVER_LOG_PATH="${SERVER_LOG_PATH:-$ROOT_DIR/logs/qwen2_72b_4bit_vllm.log}"
SERVER_PID_FILE="${SERVER_PID_FILE:-$ROOT_DIR/logs/qwen2_72b_4bit_vllm.pid}"
...
Defaults:
  ...
  DATASET_NAME=loong
  WORKER_COUNT=8
  API_MODEL_NAME=Qwen2-72B-Instruct-AWQ
  QUANTIZATION=awq
  STRUCTRAG_ENABLE_THINKING=0
  OUTPUT_PATH_SUFFIX=qwen2-72b-4bit-full
  MAX_MODEL_LEN=32768
...
Examples:
  bash scripts/72b_4bit/run_inference_full.sh
  bash scripts/72b_4bit/run_inference_full.sh --logging
  FORCE_NEW_RUN=1 bash scripts/72b_4bit/run_inference_full.sh
  OUTPUT_PATH_SUFFIX=qwen2-72b-4bit-full-v2 bash scripts/72b_4bit/run_inference_full.sh
EOF
}
...
echo "Starting full Loong dataset run with Qwen2-72B-Instruct-AWQ (4-bit)."
