#!/usr/bin/env bash
# Reverse-order inference: processes Loong samples from 1599→0.
# Intended to run in parallel with run_inference_full.sh on a second server
# so both ends converge toward the middle.
...
OUTPUT_PATH_SUFFIX="${OUTPUT_PATH_SUFFIX:-qwen35-think-off-vllm-reverse}"
...
RESULT_FULL_DIR="${RESULT_FULL_DIR:-$ROOT_DIR/result_full/qwen35_27b_vllm_reverse}"
...
Usage:
  ...
  bash scripts_full/qwen3p5_27b_vllm/run_inference_full_reverse.sh --logging
  bash scripts_full/qwen3p5_27b_vllm/run_inference_full_reverse.sh --dry-run
  bash scripts_full/qwen3p5_27b_vllm/run_inference_full_reverse.sh --smoke

Behavior:
  - Same as run_inference_full.sh but passes --reverse to main.py.
  - Processes Loong samples in REVERSE order (1599→0).
  - Run this on a second server alongside the forward run to converge from both ends.
  - Stop whichever server reaches the meeting point first.
...
Flags:
  ...
  --smoke                   Run 2 samples on worker 0 (in reverse).
...
setup_logging_mode() {
    local logging_root="${STRUCTRAG_LOGGING_ROOT:-$ROOT_DIR/27b_vllm_full_logging_reverse}"
...
download_model_if_missing() {
    if [[ "$AUTO_DOWNLOAD_MODEL" != "1" ]]; then
        ...
        echo "Example:"
        echo "  MODEL_DIR=/path/to/Qwen3.5-27B bash scripts_full/qwen3p5_27b_vllm/run_inference_full_reverse.sh"
        exit 1
    fi
...
resolve_result_dir() {
    ...
    find "$eval_root" -mindepth 1 -maxdepth 1 -type d \
        -name "${DATASET_NAME}_*reverse*" -printf '%T@ %p\n' 2>/dev/null \
        | sort -nr \
        | awk 'NR==1 {sub(/^[^ ]+ /, ""); print}'
}
...
if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "=== DRY RUN (reverse): validation only ==="
...
echo "=== Qwen3.5 27B vLLM full Loong inference (REVERSE ORDER: 1599→0) ==="
echo "model_dir=$MODEL_DIR"
echo "full_data_path=$FULL_DATA_PATH"
echo "cuda_visible_devices=$CUDA_VISIBLE_DEVICES"
echo "worker_count=$WORKER_COUNT"
...
echo "reverse=1 (samples processed 1599→0, reverse_global=1600-global printed)"
