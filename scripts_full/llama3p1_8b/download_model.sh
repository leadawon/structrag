...
MODEL_DIR="${MODEL_DIR:-$ROOT_DIR/model/Llama-3.1-8B-Instruct}"
...
usage() {
    cat <<EOF
Usage:
  bash scripts_full/llama3p1_8b/download_model.sh
  bash scripts_full/llama3p1_8b/download_model.sh --dry-run

Downloads meta-llama/Llama-3.1-8B-Instruct into StructRAG's model directory.
Requires a Hugging Face token with access to Llama 3.1 (gated model).
...
Examples:
  HF_TOKEN=hf_xxx bash scripts_full/llama3p1_8b/download_model.sh
  MODEL_DIR=/workspace/models/Llama-3.1-8B-Instruct bash scripts_full/llama3p1_8b/download_model.sh
  HF_TOKEN=hf_xxx HF_TRANSFER=1 bash scripts_full/llama3p1_8b/download_model.sh
EOF
}
...
if [[ ! -x "$PYTHON_BIN" ]]; then
    ...
    echo "Ensure huggingface_hub is installed:"
...
if [[ -n "${HF_TOKEN:-}" ]]; then
    ...
    echo "hf_token=<not set — required for gated Llama model>"
    if [[ "$DRY_RUN" -eq 0 ]]; then
        echo "Set HF_TOKEN=hf_xxx before running."
        echo "Visit https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct to request access."
        exit 1
    fi
fi
echo ""
...
echo "  bash scripts_full/llama3p1_8b/run_server.sh --detach"
echo "Then run inference:"
echo "  bash scripts_full/llama3p1_8b/run_inference_exper_full.sh"
