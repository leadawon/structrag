...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    cat <<EOF
Usage:
  bash scripts/7b/run_inference_learned_router.sh
  bash scripts/7b/run_inference_learned_router.sh sample5
  bash scripts/7b/run_inference_learned_router.sh sample100
  bash scripts/7b/run_inference_learned_router.sh sample9999
  bash scripts/7b/run_inference_learned_router.sh single <dataset_id>

Behavior:
  - No args: runs sample5 then sample100
  - Uses the main 7B model plus an external learned router at \$ROUTER_URL

Defaults:
  MODEL_DIR=$ROOT_DIR/model/Qwen2.5-7B-Instruct
  ROUTER_URL=127.0.0.1:1226
  ROUTER_TOKENIZER_PATH=$ROOT_DIR/model/Qwen2.5-7B-Instruct
