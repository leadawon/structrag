...
DATASET_NAME="${DATASET_NAME:-loong_set4_server0}"
OUTPUT_PATH_SUFFIX="${OUTPUT_PATH_SUFFIX:-_full_remaining}"
STRUCTRAG_ENABLE_THINKING="${STRUCTRAG_ENABLE_THINKING:-0}"
SERVER_SCRIPT_PATH="${SERVER_SCRIPT_PATH:-$SCRIPT_DIR/run_server.sh}"
SERVER_LOG_PATH="${SERVER_LOG_PATH:-$ROOT_DIR/logs/qwen35_27b_vllm_server0_set4_full_remaining.log}"
SERVER_PID_FILE="${SERVER_PID_FILE:-$ROOT_DIR/logs/qwen35_27b_vllm_server0_set4_full_remaining.pid}"
...
SOURCE_PENDING_PATH="${SOURCE_PENDING_PATH:-$ROOT_DIR/result_full/qwen35_27b_vllm_sets/set4/data/loong_process.jsonl}"
MAIN_SET4_EVAL_DIR="${MAIN_SET4_EVAL_DIR:-$ROOT_DIR/eval_results/$LLM_NAME/loong_set4_ordered_set4}"
SERVER0_RESULT_DIR="${SERVER0_RESULT_DIR:-$ROOT_DIR/result_full/qwen35_27b_vllm_sets/set4_server0}"
...
Behavior:
  - Runs all unfinished set4 samples assigned to server0.
  - Writes the run to eval_results/$LLM_NAME/${DATASET_NAME}${OUTPUT_PATH_SUFFIX}.
  - Writes the prepared input and manifest to result_full/qwen35_27b_vllm_sets/set4_server0.
  - AUTO_SCORE defaults to 0 because this is a partial shard to merge back later.

Defaults:
  TAIL_COUNT=$TAIL_COUNT (0 means all remaining)
...
start_server_if_needed() {
    ...
    echo "Starting vLLM server for set4 server0 full remaining shard..."
...
main_eval_dir = Path(r"$MAIN_SET4_EVAL_DIR")
...
if source_pending_path.exists() or main_eval_dir.exists():
    base_path = full_data_path
    base_rows = [
        row
        for row in load_jsonl(base_path)
        if int(row.get("set", -1)) == 4 and str(row.get("id")) not in completed
    ]
    selected = base_rows[-tail_count:] if tail_count > 0 else base_rows
elif server0_data_path.exists():
    base_path = server0_data_path
    base_rows = [row for row in load_jsonl(base_path) if int(row.get("set", -1)) == 4]
    selected = base_rows
    ...
    raise SystemExit(
        "Cannot find the prepared set4 pending file. Copy "
        f"{server0_data_path.parent.parent} from the main server, or set "
        "SOURCE_PENDING_PATH to the main set4 pending file. "
...
manifest = {
    ...
    "purpose": "set4 server0 full remaining shard",
    "tail_count": tail_count,
    "selection_mode": "tail" if tail_count > 0 else "all_remaining",
    "worker_count": max(1, (len(selected) + 199) // 200),
...
echo "=== Preparing set4 server0 full remaining shard ==="
echo "server0_result_dir=$SERVER0_RESULT_DIR"
echo "server0_eval_dir=$SERVER0_EVAL_DIR"
echo "tail_count=$TAIL_COUNT (0 means all remaining)"
prepare_server0_tail
...
if [[ "$DRY_RUN" -eq 1 ]]; then
    echo ""
    echo "=== DRY RUN ==="
    echo "DATASET_NAME=$DATASET_NAME EVAL_DATA_PATH=$SERVER0_DATA_PATH WORKER_COUNT=$WORKER_COUNT OUTPUT_PATH_SUFFIX=$OUTPUT_PATH_SUFFIX RESUME_OUTPUT_PATH_SUFFIX=$OUTPUT_PATH_SUFFIX AUTO_SCORE=$AUTO_SCORE bash run_inference.sh all_workers --no_shuffle"
    exit 0
fi
...
echo ""
echo "=== set4 server0 tail shard done ==="
echo "server0_eval_dir=$SERVER0_EVAL_DIR"
echo "server0_result_dir=$SERVER0_RESULT_DIR"
