...
# Qwen2-72B-Instruct-AWQ (4-bit AWQ quantization).
# AWQ reduces memory footprint so 4 GPUs are sufficient instead of 8.
# QUANTIZATION=awq is passed to the root run_server.sh via env var.
...
if [[ -z "${MODEL_DIR:-}" ]]; then
    MODEL_CANDIDATES=(
        "$ROOT_DIR/model/Qwen2-72B-Instruct-AWQ"
        "/workspace/lambo/models/Qwen2-72B-Instruct-AWQ"
        "/workspace/LAMBO/models/Qwen2-72B-Instruct-AWQ"
        "/workspace/qwen/Qwen2-72B-Instruct-AWQ"
    )
...
DEFAULT_LOG_PATH="$ROOT_DIR/logs/qwen2_72b_4bit_vllm.log"
DEFAULT_PID_FILE="$ROOT_DIR/logs/qwen2_72b_4bit_vllm.pid"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-32768}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-Qwen2-72B-Instruct-AWQ}"
QUANTIZATION="${QUANTIZATION:-awq}"
# AWQ sets its own internal dtype; leave DTYPE unset unless explicitly overridden
DTYPE="${DTYPE:-}"
...
usage() {
    cat <<EOF
Usage:
  bash scripts/72b_4bit/run_server.sh
  bash scripts/72b_4bit/run_server.sh --detach
  bash scripts/72b_4bit/run_server.sh --stop
  bash scripts/72b_4bit/run_server.sh --logging
  bash scripts/72b_4bit/run_server.sh --logging --detach
  bash scripts/72b_4bit/run_server.sh --logging --stop
...
Defaults:
  ...
  MAX_MODEL_LEN=32768
  SERVED_MODEL_NAME=Qwen2-72B-Instruct-AWQ
  QUANTIZATION=awq
  DTYPE=<unset; AWQ determines its own dtype>
...
Logging mode:
  STRUCTRAG_LOGGING_ROOT=$ROOT_DIR/72b_4bit_logging
EOF
}
...
setup_logging_mode() {
    local logging_root="${STRUCTRAG_LOGGING_ROOT:-$ROOT_DIR/72b_4bit_logging}"
...
if [[ ! -e "$MODEL_DIR" ]]; then
    ...
    echo "  bash scripts/72b_4bit/download_model.sh"
    exit 1
fi
